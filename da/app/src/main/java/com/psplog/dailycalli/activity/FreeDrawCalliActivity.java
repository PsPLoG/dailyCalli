...
import com.psplog.dailycalli.tool.ShapeAdapter;
...
public class FreeDrawCalliActivity extends Activity {
    ...
    private final int MODE_SHAPE_OBJ = 5;
    private final int MODE_LINE_OBJ = 6;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_freedraw_calli);
        mContext = this;
        ...
        // Create SpenNoteDoc.
        try {
            mSpenNoteDoc = new SpenNoteDoc(mContext, mScreenRect.height(), mScreenRect.height());
        } catch (IOException e) {
...
//            addImgObject(mScreenRect.width() / 3, mScreenRect.height() / 3);
//            addTextObject(mScreenRect.width() / 3 + 110, mScreenRect.height() / 3, "test");
//            addStrokeObject(mScreenRect.width() / 3 + 250, mScreenRect.height() / 3 + 50);
//            addShapeObject(mScreenRect.width() / 3, mScreenRect.height() / 3 + 250, SpenObjectShape.TYPE_HEART, "LIKE");
//            addLineObject(mScreenRect.width() / 3 + 110, mScreenRect.height() / 3 + 200, SpenObjectLine.TYPE_STRAIGHT);
...
//        textInfo.size = Math.round(18 * mCanvasWidth / 360);
            ...
            if (event.getAction() == MotionEvent.ACTION_UP && event.getToolType(0) == mToolType) {
                ...
                if (control == null) {
                    ...
                    if (mMode == MODE_IMG_OBJ) {
                        addImgObject(canvasPos.x, canvasPos.y);
                        return true;
                        ...
                        SpenObjectTextBox obj = addTextObject(canvasPos.x, canvasPos.y, null);
                        ...
                        addShapeObject(canvasPos.x, canvasPos.y, mShapeObjNumber, "");
                        return true;

                    } else if (mMode == MODE_LINE_OBJ) {
                        addLineObject(canvasPos.x, canvasPos.y, mShapeObjNumber);
                        return true;
                    }
                }
            }
    ...
    private void addImgObject(float x, float y) {
        ...
        RectF rect = new RectF(x - imageBitmap.getWidth() / 2, y - imageBitmap.getHeight() / 2, x
                + imageBitmap.getWidth() / 2, y + imageBitmap.getHeight() / 2);
    ...
    private SpenObjectShape addShapeObject(float x, float y, int type, String str) {
        ...
        try {
            ...
            Toast.makeText(mContext, "Not supported shape type: " + type, Toast.LENGTH_LONG).show();
            return null;
        }
        ...
        SpenFontSizeSpan span = new SpenFontSizeSpan(0, str.length(), SpenTextSpanBase.SPAN_INCLUSIVE_INCLUSIVE, 36);
        shapeObj.appendTextSpan(span);
    ...
    private SpenObjectLine addLineObject(float x, float y, int type) {
        RectF rect = new RectF(x - 150, y - 100, x + 150, y + 100);

        SpenObjectLine lineObj = null;
        try {
            lineObj = new SpenObjectLine(type, new PointF(rect.left, rect.top), new PointF(rect.right, rect.bottom));
        } catch (Exception e) {
            Toast.makeText(mContext, "Not supported line type: " + type, Toast.LENGTH_LONG).show();
            return null;
        }

        SpenLineStyleEffect lineStyle = new SpenLineStyleEffect();
        lineStyle.setWidth(6);
        lineObj.setLineStyleEffect(lineStyle);
...
//        dlgSave.show();
    ...
    private final SpenControlListener mControlListener = new SpenControlListener() {
        ...
        public boolean onCreated(ArrayList<SpenObjectBase> objectList, ArrayList<Rect> relativeRectList,
            ...
            if (objectList.size() == 1
                    && (objectList.get(0).getType() == SpenObjectBase.TYPE_SHAPE || objectList.get(0).getType() == SpenObjectBase.TYPE_LINE)) {
    ...
    private void initShapeSelectionDialog() {
        ...
        gridShapes.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                if (position < SHAPE_NUM) {
                    mMode = MODE_SHAPE_OBJ;
                    mShapeObjNumber = position + 1;
                    addShapeObject(200, 200, mShapeObjNumber, "");
                    mShapeSelectionDialog.dismiss();
                } else {
                    mMode = MODE_LINE_OBJ;
                    mShapeObjNumber = position - SHAPE_NUM;
                    addLineObject(200, 200, mShapeObjNumber);
                    mShapeSelectionDialog.dismiss();
                }
            }
        });
    ...
    private void enableButton(boolean isEnable) {
        ...
        mAddPageBtn.setEnabled(isEnable);
    }
